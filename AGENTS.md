...
This agent uses **mem-beads** for structured memory. After EVERY response, write a bead using the invisible marker format:

```
{::bead type="<type>" title="<short title>" summary="<summary>" session="<session-id>" turn="<turn-number>" /::}
```

**Write a bead EVERY turn**, even routine ones. The post-session extractor will parse these markers.

**IMPORTANT:** Use the `{::bead ... /::}` format — it won't render in any chat client (unlike HTML comments which show up in Telegram/Discord).
...
After significant turns, consider writing a memory bead. See `skills/mem-beads/SKILL.md` for full rules.

**Quick reference:**
```bash
export PATH="/home/node/.openclaw/workspace/tools/mem-beads:$PATH"
mem-beads create --type <type> --title "..." --summary "..." --session <id> --scope <scope> --tags "..."
```
