"""Read-only Assembly Depth measurement for SOUL and continuity surfaces."""
...
def _edge_bonus_from_manifest(root: str | Path) -> dict[str, float]:
    try:
...
def compute_assembly_depth(
    ...
    *,
    ...
    limit: int = 200,
    edge_bonus: dict[str, float] | None = None,
) -> dict[str, Any]:
    ...
    edge_bonus = dict(edge_bonus if edge_bonus is not None else _edge_bonus_from_manifest(root))
