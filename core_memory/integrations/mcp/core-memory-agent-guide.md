...
- Read the user's free-text reply and map it to exactly one `review_prompt.resolutions[].choice` id: `prefer_a`, `prefer_b`, `retract_both`, or `defer`. Use your judgment — "it's the newer one", "we're still on Postgres", "drop both", "not now" all map cleanly.
