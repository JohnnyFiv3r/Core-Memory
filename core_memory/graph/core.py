"""Internal graph implementation module.

This file houses the authoritative graph logic used by the split public modules
(`graph.structural`, `graph.traversal`, `graph.semantic`) and the compatibility
facade (`graph.api`).
"""
...
def sync_structural_pipeline(root: Path, *, apply: bool = False, strict: bool = False) -> dict:
    ...
    index = json.loads(index_file.read_text(encoding="utf-8"))
    index_before = json.dumps(index, sort_keys=True, separators=(",", ":"))

    scanned_assoc, links_added = _sync_associations_to_links(index, rel_map)
    index_after = json.dumps(index, sort_keys=True, separators=(",", ":"))
    index_changed = index_after != index_before
