...
def process_side_effect_event(*, root: str | Path, kind: str, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if k == "association-pass":
        ...
        out = run_association_coverage(
            ...
            policy_version=str(p.get("policy_version") or "bead_association.v2"),
            prompt_version=str(p.get("prompt_version") or "association_judge.v2"),
            rubric_version=str(p.get("rubric_version") or "association_truth.v2"),
            ...
            source_ingest_envelope_refs=[x for x in (p.get("source_ingest_envelope_refs") or []) if isinstance(x, dict)],
            sweep_info={
                "sweep": bool(p.get("sweep")),
                "sweep_mode": str(p.get("sweep_mode") or ""),
                "sweep_cursor": p.get("sweep_cursor"),
                "sweep_limit": p.get("sweep_limit"),
                "next_sweep_cursor": p.get("next_sweep_cursor"),
