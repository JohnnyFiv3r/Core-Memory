...
def add_graph_parser(subparsers: argparse._SubParsersAction, *, legacy_help: str) -> argparse.ArgumentParser:
    ...
    g_backfill_causal = graph_sub.add_parser("backfill-causal-links", help="Deprecated causal-link candidate generator (never writes edges)")
    ...
    g_causal_candidates = graph_sub.add_parser("causal-candidates", help="Generate causal-link candidates for association-agent judgment")
    g_causal_candidates.add_argument("--max-per-target", type=int, default=3)
    g_causal_candidates.add_argument("--min-overlap", type=int, default=2)
    g_causal_candidates.add_argument("--no-require-shared-turn", action="store_true")
    g_causal_candidates.add_argument("--bead-id", action="append", help="Limit candidates to pairs touching these bead IDs")
    g_causal_candidates.add_argument("--bead-ids-file", help="Path to JSON array of bead IDs for targeted mode")
