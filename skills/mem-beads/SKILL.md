...
**Link Types (from mem-beads-links-spec.md):**

| Type | Direction | Meaning | Cycles? |
|------|-----------|---------|---------|
...
| `revises` | source → target | Corrects or contradicts earlier | No |
...
**Cycle Detection:** `follows`, `derives-from`, `extends`, `supersedes`, `revises` must remain acyclic. Adding an edge that would create a cycle is rejected.
