...
export class OpenAIRealtimeSession {
    ...
    constructor(emit) {
        ...
        if (!apiKey)
            throw new Error("OPENAI_API_KEY is required for realtime");
                    ...
                    tools: [
                        {
                            type: "function",
                            name: "select_story",
                            description: "Select the most relevant John Inniger portfolio story only when user explicitly asks for examples/background/project context. Return none when not needed.",
                            parameters: {
                                type: "object",
                                properties: {
    ...
    handleSelectStoryToolCall(callId, argsRaw) {
        if (!callId)
            return;
        let args = {};
        try {
    ...
    handleUpstreamMessage(json) {
        ...
        if (event.type === "response.text.delta") {
            ...
            if (text)
                this.emit({ type: "assistant.text.delta", text });
            return;
        }
        if (event.type === "response.text.done") {
            const text = String(event.text ?? "").trim();
            if (text)
                this.emit({ type: "assistant.text.final", text });
            return;
        }
