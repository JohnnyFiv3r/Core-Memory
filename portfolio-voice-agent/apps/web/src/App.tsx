...
export function App() {
  ...
  if (isEmbed) {
    return (
      <main className="relative grid h-full w-full place-items-center overflow-visible bg-transparent px-8 py-2 text-foreground">
        <div className={`grid w-full max-w-sm justify-items-center gap-0.5${sessionLimitReached ? " pointer-events-none blur-sm" : ""}`}>
          <div
            className="relative grid place-items-center overflow-hidden rounded-full"
            style={{ height: "min(56vh, 360px)", width: "min(90vw, 360px)" }}
          >
            <PersonaOrb className="origin-center scale-[1.87] max-sm:scale-[1.53]" state={state} />
          ...
          <p className="max-w-sm text-center text-sm leading-tight text-muted-foreground mt-0">Tap the microphone button to chat with my voice agent.</p>
