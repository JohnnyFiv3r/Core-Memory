...
def test_visible_corpus_indexes_source_turn_transcript_before_candidate_selection(tmp_path: Path):
    from core_memory.runtime.turn_archive import append_turn_record

    s = MemoryStore(root=str(tmp_path))
    bid = s.add_bead(
        type="context",
        title="Gina [other]: Wow, nice spot! Where is it?",
        summary=["Gina asks about the spot."],
        session_id="bench:locomo:conv-30:replay",
        source_turn_ids=["locomo:conv-30:D2:5:5"],
    )
    append_turn_record(
        root=tmp_path,
        session_id="bench:locomo:conv-30:replay",
        turn_id="locomo:conv-30:D2:5:5",
        transaction_id="tx-1",
        trace_id="tr-1",
        origin="BENCHMARK_REPLAY",
        ts="2:32 pm on 29 January, 2023",
        user_query="",
        ...
        turns=[
            {
                "speaker": "Gina",
                "role": "other",
                "content": "Paris?! That is really great Jon! Never had a chance to visit it. Been only to Rome once.",
                "metadata": {
                    "locomo_session_date_time": "2:32 pm on 29 January, 2023",
                    "blip_caption": "photo of a dance studio in Paris",
                },
            }
        ],
        speakers=["Gina"],
        assistant_final_ref=None,
        assistant_final_hash="",
        tools_trace=[],
        mesh_trace=[],
        metadata={"locomo_session_date_time": "2:32 pm on 29 January, 2023"},
    )

    rows = build_visible_corpus(tmp_path)
    row = next(r for r in rows if r["bead_id"] == bid)

    assert "Paris" in row["semantic_text"]
    assert "Rome" in row["semantic_text"]
    assert "29 January, 2023" in row["semantic_text"]
    assert "Paris" in row["lexical_text"]
