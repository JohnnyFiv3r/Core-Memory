...
class TestQdrantEmbeddedBackend(unittest.TestCase):
    ...
    def _point(self, bead_id: str, eligible: bool = True, status: str = "active") -> dict:
        return {
            "bead_id": bead_id,
    ...
    def test_filtered_search_excludes_retracted(self):
        active = self._point("a1", status="active")
        retracted = self._point("r1", status="retracted")
        self.backend.upsert(**{k: v for k, v in active.items() if k != "bead_id"}, bead_id=active["bead_id"])
