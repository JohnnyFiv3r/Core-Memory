...
PACK = (
    ...
    / "packs"
    / "public"
    / "observations-statement-request-v1"
)
...
def test_statement_request_pack_has_25_accepted_cases_without_runtime_gold_leakage():
    ...
    assert validation.bucket_counts["statement_request"] == 25
    assert validation.bucket_counts["statement"] == 13
    assert validation.bucket_counts["request"] == 12

    cases = _load("inputs/cases.json")
    assert [row["case_id"] for row in cases] == [f"obs-sr-{index:03d}" for index in range(1, 26)]
    for case in cases:
...
def test_statement_request_gold_is_observation_first_and_exactly_evidence_bound():
    cases = _indexed(_load("inputs/cases.json"))
    gold = _indexed(_load("gold/gold.json"))

    assert set(gold) == set(cases)
    preferred_counts = {"statement": 0, "request": 0}
    for case_id, expected in gold.items():
        ...
        assert set(expected["acceptable_labels"]) <= {"statement", "request", "reflection"}
        ...
        assert len(expected["evidence_spans"]) == len(evidence_by_id) == 2
    ...
    assert preferred_counts == {"statement": 13, "request": 12}


def test_statement_request_manifest_checksums_and_adjudications_are_exact():
    ...
    assert manifest["pack_id"] == "public-observations-statement-request-v1"
    ...
    assert sum(row["human_decision"] == "accepted" for row in adjudications.values()) == 21
    assert sum(row["human_decision"] == "revised" for row in adjudications.values()) == 4
