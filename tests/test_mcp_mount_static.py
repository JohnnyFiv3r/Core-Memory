...
class MCPMountStaticTests(unittest.TestCase):
    ...
    def test_http_server_mounts_mcp_subapp(self):
        ...
        self.assertIn("build_mcp_app", text)
        self.assertIn("set_mcp_request_root", text)
        self.assertIn("reset_mcp_request_root", text)
        self.assertIn("return build_mcp_app(root=server_root, lock_root=hosted)", text)
        self.assertIn('headers.get(b"x-tenant-id"', text)
