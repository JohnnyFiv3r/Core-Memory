...
Weak beads and missing causal edges in Satorid are expected under the current
...
11. Provide a safe append-only Satorid backfill path that preserves source
...
the hosted Satorid path receives full-schema delegated authorship.
...
### Slice 7 — Governed reauthoring and Satorid backfill
...
**Exit condition:** Satorid no longer depends on legacy deterministic semantics,
...
### 15.3 Satorid migration

The current Satorid evidence-anchor beads remain immutable provenance records.
...
9. **Append-only migration:** Satorid source anchors remain intact; improved
...
11. Satorid backfill completes without mutating original provenance;
