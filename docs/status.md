...
| 6 | Watershed attribution over the roadmap | **Shipped** — `core_memory/graph/roadmap.py`, `core_memory/retrieval/roadmap.py`, and `/v1/memory/projection/junction-roadmap/attribution` in `core_memory/integrations/http/server.py`; `tests/test_junction_roadmap.py` and `tests/test_http_contract_async_ops.py` cover roadmap-watershed attribution, source scoping, stale receipts, and the HTTP contract |
| 7 | Seam healing, PER-linked `validated_outcome` writeback, and path promotion | **Partially shipped** — validated stitched plans can be submitted through `maintain(action="propose_seam_healing_candidates")` to create Dreamer review candidates only (`core_memory/runtime/dreamer/seam_healing.py`; `tests/test_seam_healing_candidates.py`). The planner remains read-only except for endpoint-rich seam receipts. PER-linked `validated_outcome` rewards and stitched-path promotion remain unimplemented |
