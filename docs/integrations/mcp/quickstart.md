...
Do not call it when sync is not enabled, when the user has opted out, or when opt-in is unclear; ask first. Keep the stable conversation/session identity the same across snapshots for the same conversation. Send an `idempotency_key` for the exact snapshot when the client has one; replaying the same key/content returns `duplicate=true` rather than writing again. Include only visible conversation content intended for memory sync, not hidden instructions, credentials, or unrelated private data. It returns a `transcript_hash` that can be passed as `previous_snapshot_hash` on the next snapshot.
