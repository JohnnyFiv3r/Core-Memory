...
- `core_memory.maintain(...)` — governed control-plane facade for management actions (approval, cleanup, async ops, association dispatch, association coverage review, Dreamer candidate review, seam-healing candidate submission, SOUL revisions, Myelination refresh, and correction/re-review actions). Mutating dry-runs return `required_authority`, `authority_ok`, and validation errors.
...
- `core_memory.tombstone_bead(...)` — governed single-bead semantic wrapper over `remove_beads`: retires one mistaken bead from active recall/graph truth while preserving the `bead_removed` tombstone, association pruning, and rebuild exclusion. Requires a single `bead_id` + reason; rejects multi-bead targets
...
- Canonical semantic and hybrid reads index cited chunk turns as evidence-only
  ...
  bead before the result can enter filtering, reranking, or graph traversal.
  Chunk and vector IDs never surface as causal bead IDs.
