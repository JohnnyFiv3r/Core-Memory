...
| `core_memory/retrieval/pipeline/explain.py` | Artifact debt / compatibility shim | Retains old `build_explain(...)` path; live explain payload is built in `retrieval/pipeline/__init__.py` | Inline explain construction in `core_memory.retrieval.pipeline` | Remove only after `rg "retrieval.pipeline.explain\|build_explain" core_memory tests docs benchmarks eval scripts plugins` returns no active callers outside historical docs/ledger and release notes call out the path removal. | `tests/test_explain_contains_snaps_and_scores.py`, import scan before deletion |
| `core_memory/persistence/store_core_delegates_mixin.py` and `core_memory/persistence/store_reporting_promotion_mixin.py` | Artifact debt / private | Retained mixin files from the flattened `MemoryStore` MRO. They are not currently imported by first-party runtime code. | `core_memory/persistence/store.py` plus `store_*_ops.py`, `promotion_service.py`, and reporting modules | Delete in a proven-dead artifact PR only after docs are updated in the same PR and `rg "StoreCoreDelegatesMixin\|StoreReportingPromotionMixin" core_memory tests` returns no active imports. | `pytest -m mixin_assembly`, `tests/test_store_core_delegates_mixin.py`, import scan |
...
| `core_memory/persistence/write_ops.py` | Artifact debt / compatibility shim | Thin delegates for old write-operation imports | Direct `MemoryStore` methods and canonical runtime write boundaries (`process_turn_finalized`, `process_flush`) | Remove only after active import scan is clean and a breaking-change/deprecation note covers the old module path. | Import scan, `tests/test_package_root_public_surface.py`, focused write-path tests |
