...
| `core-memory graph backfill-causal-links --apply` | Public CLI compatibility | Previously applied lexical-overlap-derived `supports`/`derived_from` links and structural edges directly. That deterministic semantic mutation conflicts with the agent-led write invariant. | Relationship-neutral candidate generation followed by the governed association agent judge/apply flow (`maintain` association actions) | `1.1.1` is the compatibility window: `--apply` is candidate-only and emits warning telemetry without writing links. In `1.2.0`, reject the flag with migration guidance. The cutover must include focused CLI compatibility tests. | `tests/test_graph_r2.py`, association coverage/judge tests, CLI parser/handler tests, `scripts/check_architecture_guards.py --fail-on-new-compat` |
| `POST /v1/memory/hygiene/seed-backfill` and `run_seed_quality_backfill(..., apply=True)` | Public HTTP and private one-shot compatibility | The temporary seed cleanup directly rewrote legacy bead/entity fields and auto-applied Dreamer outcomes. That mutating behavior conflicts with append-only agent-led semantic authority. The route now retains a read-only legacy quality census; `apply=true` is rejected. | Governed `maintain(action="reauthor_memory")`, `maintain(action="retry_pending_semantic")`, and `maintain(action="semantic_backfill_report")` | Keep the read-only route through `1.1.1` with `seed_quality_backfill_apply_retired` migration guidance. Remove the route/module in `1.2.0` after an external-caller review and proof that operator docs use the governed copied-tenant-first workflow. The mutating behavior must not return. | `tests/test_seed_quality_backfill.py`, `tests/test_semantic_reauthoring.py`, HTTP contract validation, `scripts/check_architecture_guards.py --fail-on-new-compat` |
| Association candidate/judge v1 records (`memory.association_candidates.v1`, `memory.association_judge.v1`) | Public persisted-data and task compatibility | V1 candidate rows could contain engine-proposed relationship, direction, and confidence values. Reusing those defaults for new writes violates agent-led semantic authority. | `memory.association_candidates.v2` relationship-neutral pairs plus `memory.association_judge.v2` decisions that explicitly author relationship, direction, evidence, confidence, reason, and truth basis | New candidate generation emits v2 only. Existing stored v1 candidates remain readable through `1.1.1`, but their proposed fields are diagnostic metadata only. Remove the v1 reader in `1.2.0` only after a persisted-store audit and a documented migration/review path for unresolved v1 candidates. | `tests/test_association_coverage.py`, HTTP contract tests, `scripts/check_architecture_guards.py --fail-on-new-compat` |
| Implicit agent-authored mode default and deterministic blocked-write stub | Public runtime configuration and write behavior | Earlier releases defaulted `CORE_MEMORY_AGENT_AUTHORED_MODE` to `warn`; blocked writes could create a deterministic context stub. | `hard` is now the implicit default. Missing/invalid writes retain the raw turn plus pending-semantic state; explicit `warn` or `off` preserves compatibility behavior. Governed repair is opt-in through `CORE_MEMORY_AGENT_AUTHORED_REPAIR=1` or runtime policy. | Keep `warn`, `off`, and legacy `enforce`/`observe` aliases through `1.x`. Remove them in `2.0.0` only after adapter migration guidance and a staged submission-success review. | `tests/test_hard_agent_authorship.py`, `tests/test_agent_authored_runtime_gate.py`, `tests/test_f_w2_gate_severity.py`, `tests/test_semantic_write_receipts.py` |
| Authored-update ingress aliases (`metadata.crawler_updates`, `bead_judge=llm`) | Public adapter/request compatibility | Lets pre-contract adapters supply updates through metadata or request delegated full-schema authorship through the former judge directive. Typed top-level input wins and the aliases emit deprecation diagnostics. | Top-level `crawler_updates` plus `authoring_mode=inline|delegated` | `1.1.1` is the compatibility window. Stop accepting both aliases in `1.2.0` after hosted bridge/adapters are verified on typed ingress and no active first-party caller remains. | `tests/test_agent_authored_typed_ingress.py`, `tests/test_turn_memory_authoring.py`, `scripts/check_architecture_guards.py --fail-on-new-compat` |
| Event-only Python `emit_turn_finalized(...)` facade | Public Python compatibility | Emits a durable finalized-turn event and opens pending semantic state but cannot return the processed v2 semantic receipt. | Processed `write_turn_finalized(...)` returning `memory.turn_finalized_receipt.v2` | Keep through `1.x`. Remove in `2.0.0` after public examples/adapters use the processed facade or an explicitly queued API and the persisted-event migration gate passes. | `tests/test_semantic_write_receipts.py`, `tests/test_integrations_api.py`, `tests/test_package_root_public_surface.py` |
